...
/**
 * MagicBento (react-bits, adapted for SnapList's Prism-light identity).
 * Changes from upstream: white cards on the light canvas (was near-black),
 * violet glow tuned to --color-iris, card content comes in via a `cards`
 * prop instead of hardcoded demo data, the global spotlight lost its
 * `mix-blend-mode: screen` (invisible on white) in favor of a soft lavender
 * tint, and the dead non-particle render branch was dropped.
 */
...
export interface BentoProps {
  ...
  enableTilt?: boolean;
  /** "r, g, b" — defaults to the iris violet. */
  glowColor?: string;
  clickEffect?: boolean;
...
const GlobalSpotlight: React.FC<{
  ...
  useEffect(() => {
    ...
    // Light-theme spotlight: no screen blend (invisible on white) — just a
    // soft lavender pool that follows the cursor.
...
const useMobileDetection = () => {
  ...
  useEffect(() => {
    ...
    // rAF-deferred so the initial sync read doesn't set state mid-effect
    // (react-hooks/set-state-in-effect — same pattern as app-sidebar).
        ...
        {`
          ...
          .card--border-glow::after {
            ...
            background: radial-gradient(var(--glow-radius) circle at var(--glow-x) var(--glow-y),
                rgba(${glowColor}, calc(var(--glow-intensity) * 0.55)) 0%,
