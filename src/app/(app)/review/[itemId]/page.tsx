...
export default async function ReviewPage({
  ...
  // Seller pricing strategies (quick/balanced/maximize) — positions in the real
  // comp band. Pure + honesty-gated: a tight/low-confidence tier yields one point.
  const strategies =
    suggested != null
      ? deriveStrategies({
          suggested,
  ...
  // Dynamic per-product clarify options — generated only for a non-high estimate we
  // can still sharpen, and only when parsed attributes exist. Best-effort: the
  // generator returns [] on any failure, so the review page never blocks on it.
  // (One model call per low-confidence review render; move to on-demand/cache if it
