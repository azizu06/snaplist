...
/**
 * Keep only comps that match the item identity, aren't accessories/parts, aren't
 * multi-unit lots, and aren't new/sealed when the seller's item is used. The
 * legacy per-criterion boolean helpers this delegated to were superseded by
 * `selectSoldCompEvidence` (`./sold-comp-matcher`), which implements the same
 * relevance criteria as a single ranked matcher; they were removed as dead code
 * once nothing outside their own definitions still referenced them.
 */
