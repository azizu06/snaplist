...
describe("buildPredictionLogRow", () => {
  ...
  it("retains durable sold-date and approved-provider evidence in prediction logs", () => {
    const soldAt = Date.parse("2026-07-10T12:00:00.000Z");
    ...
    const result = makeResult({
      price: {
        ...
        tier: "ebay-sold",
        sources: [{
          url: "https://www.ebay.com/itm/persisted-sold-comp",
    ...
    const checkpointedSources = JSON.parse(JSON.stringify(
      buildPredictionLogRow("user-1", "item-1", result).sources,
    ));

    expect(checkpointedSources).toEqual(result.price.sources);
  });
