...
/**
 * Batch-sign paths in ONE storage round-trip, returning a path → signed-url Map.
 * PER-ENTRY failures (missing/unauthorized object) are dropped — the path is
 * simply absent from the map — but a BATCH/transport failure (storage outage,
 * network error) THROWS. The strict-transport policy for flows that must not
 * mistake an outage for missing photos: the eBay publish path uses this so a
 * transient storage error surfaces as a retryable failure instead of marking
 * the listing failed with a "re-upload your photos" message.
 */
export async function batchSignPhotoUrls(
  ...
  const { data, error } = await store.createSignedUrls(paths, expiresIn);
  if (error) {
    throw new Error(`Failed to batch-sign photos: ${error.message}`);
  }
 ...
 * review media card, export/publish previews): `batchSignPhotoUrls` with the
 * transport failures swallowed too. A missing thumb renders as a placeholder —
 * unlike the vision call, where a missing image would silently shrink the
 * model's input, so `resolvePhotoImages` above throws instead. Never throws;
 * a storage outage yields an empty map (the page still renders).
 */
export async function signPhotoUrlMap(
  ...
  try {
    return await batchSignPhotoUrls(supabase, paths, options);
  } catch {
