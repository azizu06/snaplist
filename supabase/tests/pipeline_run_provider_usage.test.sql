...
-- #1138 installs on top of the copy above, same convention: a shared local
-- stack that is behind this branch gets the new validators and merge inside the
-- contract's own transaction, and `post-completion-migration.test.ts` holds this
-- copy byte-identical to the file it names.
-- >>> BEGIN inline copy of supabase/migrations/20260921000000_sold_comp_usage_reasons.sql
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Issue #1138: a SECOND usage write must carry `accepted` and `reason` through
-- the merge, and must never DOWNGRADE the reason.
--
-- The merge is what a guided correction, a retry, and a queue redelivery all go
-- through. Taking the last non-null reason would let a redelivery reporting
-- `no-candidates` erase an earlier `provider-error` — turning a broken provider
-- back into "this item has no comps", which is the exact confusion the field was
-- added to end. Both write orders are asserted, because only one of them fails
-- when the ranking is missing.
-- ---------------------------------------------------------------------------
select is(
  private.provider_usage_merge_sold_comps(
    '[{"strategy":"apify","attempts":1,"results":0,"accepted":0,
       "reason":"provider-error","chargedUsd":0.0001}]'::jsonb,
    '[{"strategy":"apify","attempts":1,"results":0,"accepted":0,
       "reason":"no-candidates","chargedUsd":0.0001}]'::jsonb
  )->0->>'reason',
  'provider-error',
  'a later no-candidates cannot downgrade a stored provider-error'
);
...
select is(
  ...
  'provider-error',
  'a later provider-error replaces a stored no-candidates'
);

select is(
  private.provider_usage_merge_sold_comps(
    '[{"strategy":"apify","attempts":1,"results":4,"accepted":0,
       "reason":"all-rejected:identity-mismatch","chargedUsd":0.02}]'::jsonb,
    '[{"strategy":"apify","attempts":1,"results":6,"accepted":2,
       "reason":null,"chargedUsd":0.03}]'::jsonb
  ),
  '[{"reason": null, "results": 10, "accepted": 2, "attempts": 2,
     "strategy": "apify", "chargedUsd": 0.05}]'::jsonb,
  'a second write that anchors sums both counts and clears the reason'
);

select is(
  private.provider_usage_merge_sold_comps(
    '[{"strategy":"apify","attempts":1,"results":9,"chargedUsd":0.02}]'::jsonb,
    '[{"strategy":"apify","attempts":1,"results":3,"accepted":1,
       "reason":null,"chargedUsd":0.01}]'::jsonb
  ),
  '[{"reason": null, "results": 12, "accepted": 1, "attempts": 2,
     "strategy": "apify", "chargedUsd": 0.03}]'::jsonb,
  'a pre-#1138 stored row merges with a new one without losing either'
);

select is(
  private.provider_usage_record_is_strict(
    '{"schemaVersion":1,"modelCalls":0,"inputTokens":0,"cachedInputTokens":0,
      "outputTokens":0,"reasoningTokens":0,"models":[],
      "soldComps":[{"strategy":"ebay-sold","attempts":1,"results":2,"accepted":0,
