...
begin
  if coalesce(v_user_id, '') = ''
    or p_action not in ('prepare', 'fail')
    or p_run_id is null
...
-- unchanged-photo capability as every other guided correction. This fixed
-- internal completion makes the corrected item, prediction, included-correction
-- settlement, and replay receipt ONE transaction. A failed call therefore leaves
-- the old revision and pending claim together; no successful correction can lose
-- the receipt needed to replay it.
