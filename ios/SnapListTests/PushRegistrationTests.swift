...
/// Issue #1137. What the seller sees when a push lands with the app already open.
///
/// iOS draws it. The custom in-app banner (#891) did not look like a
/// notification (no app icon, off styling), so no in-app surface may take the
/// notification from the system.
final class ForegroundPushPresentationTests: XCTestCase {
    private let systemPresentation: UNNotificationPresentationOptions =
        [.banner, .sound, .list]

    func testAForegroundPushIsDrawnBySystem() {
        XCTAssertEqual(ForegroundPushPolicy.presentationOptions(), systemPresentation)
    }
}
