...
codebase. **Layout: single-context** — one `CONTEXT.md` + `docs/adr/` at the repo root.
...
If any of these files don't exist yet, **proceed silently**. Don't flag their absence; don't suggest
...
name), use the term as defined in `CONTEXT.md` (e.g. *PricingProvider*, *tier*, *confidence
composite*, *export pack*, *adapter*). Don't drift to synonyms the glossary explicitly avoids.
