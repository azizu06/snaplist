...
- Scope: 25 base frames plus 17 RUN/REV delta frames, for 42 implementation-frozen renderable
  frames. `REV-02` is the approved parent contract for six listed variants.

The packages are authoritative for the appearance and interaction contract of the frozen states.
...
### Implementation-frozen by the RUN/REV delta: 17 frames

| Family | IDs | Owning issue and dependencies |
| --- | --- | --- |
| Durable Runs + Recovery | `RUN-01` through `RUN-08` | [#211](https://github.com/azizu06/snaplist/issues/211), `Phase 2`, blocked by #205 and #161 |
| Identity + Guided Correction | `REV-01`, `REV-02a`, `REV-02b`, `REV-02c`, `REV-02d`, `REV-02d-retry`, `REV-02e`, `REV-07`, `REV-08` | [#212](https://github.com/azizu06/snaplist/issues/212), `Phase 1`, blocked by #205, #174, and #211 |

`REV-02` is a parent contract, not a separate rendered frame. The delta does not change #206–#209.
...
- The delta adds approved placements for `pose-01-analyzing.png` at `RUN-02`,
  `pose-03-retry-review.png` at `RUN-05`, and `pose-01-uncertain.png` at `REV-08`. Those permissions
  do not broaden placement elsewhere.
...
1. Verify the base and delta ZIP hashes, exact-base declaration, internal checksums, and relevant
   `README-FIRST.md` plus machine-readable files.
