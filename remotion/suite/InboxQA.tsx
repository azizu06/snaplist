 ...
 * math stays simple and is verified the same way by assert-clicks.ts. Only the
 * item content (image, title, price, the question, the drafted reply) differs.
 *
 * Render: npx remotion render remotion/index.ts inbox-qa public/demo/inbox-qa.mp4 --crf 26 --muted
 */

export const INBOX_QA_LEN = 660;
const SEAM = 16;
...
// Honest grounding: the draft asserts only what the photos show — the board is
// pictured, and the visible pieces have an even patina with no chips/bends it can
// see. A full count of 32 (and confirming none are hidden-damaged) is a hands-on
// check, so the draft hands that to the seller; EDIT_TEXT is the seller's own
// verified count before approving.
const DRAFT_TEXT =
  "It comes with the wooden board shown, and in the photos the brass pieces have an even, honest patina from age — no chips or bends I can spot. On whether all 32 are present and sound:";
const EDIT_TEXT = " I counted them just now and all 32 are there, intact.";

const ROW_C = center(ROW1);
const BOX_C = center(TEXTBOX);
const APPROVE_C = center(APPROVE);

export const INBOX_QA_WAYPOINTS: Array<[number, number, number]> = [
  [0, 1240, 706],
...
export const inboxQaCursorAt = (frame: number) => path(frame, INBOX_QA_WAYPOINTS);

export const INBOX_QA_CLICKS: ClickSpec[] = [
  {
    label: "inbox-qa: click the new conversation row",
    frame: CLICK_ROW,
    target: ROW1,
    arrive: ARRIVE_ROW,
    until: CLICK_ROW + 8,
  ...
  {
    label: "inbox-qa: click into the draft to edit",
    frame: CLICK_BOX,
    target: TEXTBOX,
    arrive: ARRIVE_BOX,
    until: CLICK_BOX + 8,
  ...
  {
    label: "inbox-qa: click Approve & send",
    frame: CLICK_APPROVE,
    target: APPROVE,
        ...
        >
          ...
          <div style={{ minWidth: 0, flex: 1 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
              <span style={{ fontSize: 13.5, fontWeight: 800, color: INK }}>D. Rivera</span>
...
function QaAct() {
  const frame = useCurrentFrame();
  const cursor = inboxQaCursorAt(frame);
  const press = Math.min(
    1,
    pressAt(frame, CLICK_ROW) + pressAt(frame, CLICK_BOX) + pressAt(frame, CLICK_APPROVE),
  );
  const fadeOut = interpolate(frame, [INBOX_QA_LEN - SEAM, INBOX_QA_LEN - 1], [1, 0], {
  ...
  return (
    <AbsoluteFill style={{ opacity: fadeOut }}>
      <Shell active={2} badge="LIVE INBOX · ANSWER BUYERS">
        <InboxList />
        <ThreadBody />
        <ThreadHeader />
        <Composer />
...
export const InboxQA: React.FC = () => (
  <Scene>
    <Sequence from={INBOX_QA_LEN - SEAM} durationInFrames={SEAM}>
      <Freeze frame={0}>
        <QaAct />
      </Freeze>
    </Sequence>
